...
/// Engine is the Gloomio matching engine.
/// When a caller processes a SequencedCommand on Gloomio, it uses this type.
/// process leaves the event buffer unchanged.
pub struct Engine {
    #[allow(dead_code)]
    config: EngineConfig,
}
...
mod tests {
    ...
    fn process_appends_no_events_in_this_slice() {
        // Given a Gloomio engine and an empty event buffer
    ...
    fn engine_builder_builds_engine() {
        // Given the Gloomio matching engine builder
        // When we build
        let engine = GloomioMatchingEngineBuilder::build();
